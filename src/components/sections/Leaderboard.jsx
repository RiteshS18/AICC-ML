...
function LeaderboardCard({ title, yearData, delay = 0 }) {
  return (
    <motion.div 
      ...
      className="bg-white/90 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-slate-100 flex-1 relative z-10"
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        {/* Class Standings */}
        <div>
          <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-5">Class Standings</h4>
            ...
            {yearData.classes.map((cls, idx) => (
              <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100 hover:border-primary/20 hover:shadow-sm transition-all">
                <div className="flex items-center gap-3">
                  <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${idx === 0 ? 'bg-amber-100 text-amber-700' : idx === 1 ? 'bg-slate-200 text-slate-700' : 'bg-orange-100 text-orange-700'}`}>
                    {idx + 1}
                  </span>
                  <span className="text-base font-semibold text-slate-800">{cls.name}</span>
                </div>
                <div className="text-right">
                  <span className="text-lg font-bold text-slate-900">{cls.points}</span>
        ...
        {/* Top Individuals */}
        <div>
          <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-5">Top Individuals</h4>
          <div className="space-y-4">
            {yearData.individuals.map((ind, idx) => (
              <div key={idx} className="flex justify-between items-center p-4 rounded-xl bg-slate-50 border border-slate-100 hover:border-primary/20 hover:shadow-sm transition-all">
                <div className="flex items-center gap-3">
                  <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${idx === 0 ? 'bg-amber-100 text-amber-700' : 'bg-slate-200 text-slate-700'}`}>
                    {idx + 1}
                  </span>
                  <div>
                    <p className="text-base font-bold text-slate-900">{ind.name}</p>
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-widest mt-0.5">{ind.rollno}</p>
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-xl font-bold text-slate-900">{ind.points}</span>
                  <span className="text-xs text-slate-500 font-medium ml-1">pts</span>
                </div>
              </div>
            ))}
